	...
	describe("when the DLQ queue has 2 messages", () => {
		...
		test.stdout()
			...
			.it("replays them to the main SQS queue", ctx => {
				...
				expect(ctx.stdout).to.contain(
					"finding the SQS queue [queue-dev] in [us-east-1]"
				);
				expect(ctx.stdout).to.contain(
					`replaying events from [${dlqUrl}] to [SQS:queue-dev] with 10 concurrent pollers`
				);
		...
		test.stdout()
			...
			.it("replays them to the main SQS queue in another account & region", ctx => {
				...
				expect(ctx.stdout).to.contain(
					"finding the SQS queue [queue-dev] in [eu-west-1]"
				);
				expect(ctx.stdout).to.contain(
					`replaying events from [${dlqUrl}] to [SQS:queue-dev] with 10 concurrent pollers`
				);
		...
		test.stdout()
			...
			.it("replays them to a SNS topic", ctx => {
				...
				expect(ctx.stdout).to.contain(
					"finding the SNS topic [queue-dev] in [us-east-1]"
				);
				expect(ctx.stdout).to.contain(
					`replaying events from [${dlqUrl}] to [SNS:queue-dev] with 10 concurrent pollers`
				);
				expect(ctx.stdout).to.contain("all done!");
		...
		test.stdout()
			...
			.it("replays them to a SNS topic in another account & region", ctx => {
				...
				expect(ctx.stdout).to.contain(
					"finding the SNS topic [queue-dev] in [eu-west-1]"
				);
				expect(ctx.stdout).to.contain(
					`replaying events from [${dlqUrl}] to [SNS:queue-dev] with 10 concurrent pollers`
				);
		...
		test.stdout()
			...
			.it("replays them to a Kinesis stream", ctx => {
				...
				expect(ctx.stdout).to.contain(
					`replaying events from [${dlqUrl}] to [Kinesis:queue-dev] with 10 concurrent pollers`
				);
				expect(ctx.stdout).to.contain("all done!");
