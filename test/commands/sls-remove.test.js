...
describe("sls-remove", () => {
	test.stdout()
		...
		.it("sls-remove hello-world-dev us-east-1", ctx => {
			expect(ctx.stdout).to.contain(
				"getting the deployment bucket name for [hello-world-dev] in [us-east-1]"
			);
			expect(ctx.stdout).to.contain(
				`emptying deployment bucket [${bucketName}]...`
			);
			expect(ctx.stdout).to.contain(
				"removing the stack [hello-world-dev] in [us-east-1]..."
			);
	...
	describe("when stack is not created by SLS", () => {
		...
		const errorMessage =
			'Stack [hello-world-dev] in [us-east-1] does not have a "ServerlessDeploymentBucketName", are you sure it was deployed with Serverless framework?';
		test.stdout()
			.command(["sls-remove", "-n", "hello-world-dev", "-r", "us-east-1"])
	...
	describe("when emptyS3Buckets is enabled", () => {
		describe("when there are no other buckets", () => {
			...
			test.stdout()
				...
				.it("does nothing", ctx => {
					expect(ctx.stdout).to.contain(
						"no other S3 buckets are found besides the deployment bucket"
					);
				});
		});
		...
		describe("when there are other buckets", () => {
			...
			test.stdout()
				...
				.it("empties the other buckets", ctx => {
					expect(ctx.stdout).to.contain(
						"found 2 buckets (excluding the deployment bucket)"
					);
					...
					const buckets = mockListObjectsV2.mock.calls.map(
						([{ Bucket }]) => Bucket
					);
